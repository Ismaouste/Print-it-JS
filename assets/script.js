...
dots.addEventListener("click", function (e) {
  if (e.target.classList.contains("dot")) {
    ...
    document.querySelector(
      ".banner-img"
    ).src = `assets/images/slideshow/${slides[slide].image}`;
...
function nextSlide() {
  ...
  if (currentSlide < slides.length - 1) {
    dots
      .querySelectorAll(".dot")
      [parseInt(currentSlide) + 1].classList.add("dot_selected");
    img.src = `assets/images/slideshow/${
      slides[parseInt(currentSlide) + 1].image
    }`;
...
function previousSlide() {
  ...
  if (currentSlide > 0) {
    ...
    dots
      .querySelectorAll(".dot")
      [slides.length - 1].classList.add("dot_selected");
  }
}
